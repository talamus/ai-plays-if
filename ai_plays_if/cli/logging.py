...
class ScreenFormatter(jsonlogger.JsonFormatter):
    ...
    def special_print(self, title: str, text: str) -> str:
        """For special log records we need special output."""
        COLORS = {
            "Title": Style.BRIGHT + Fore.GREEN,
            "Command": Style.BRIGHT + Fore.MAGENTA,
            "Output": Style.BRIGHT + Fore.GREEN,
        }
        color = COLORS[title] if title in COLORS and self.RESET_COLORS else ""

        pieces = []
        for piece in text.split("\n"):
            pieces.append("\n".join(textwrap.wrap(piece)))

        text = textwrap.indent("\n".join(pieces), 9 * " ").strip()
        return f"\n{color}{title}:{self.RESET_COLORS}{(8 - len(title)) * ' '}{text}"

    def format(self, record):
        """Formatting a record. Highlighting possible arguments."""

        match record.msg:
            case "Title" | "Command" | "Output":
                return self.special_print(record.msg, record.text)
            case _:
